...
# Set paths
$repo = "C:\Users\ffwfj\OneDrive\سطح المكتب\salasa5"
...
pnpm install
Write-Output "Running: pnpm run build"
pnpm run build
Pop-Location
...
if (-Not (Test-Path $dist)) {
    Write-Error "Frontend build output not found in: $dist. Make sure pnpm run build succeeded."
    exit 1
}
